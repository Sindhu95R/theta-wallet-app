...
const GroupExpenses = () => {
    ...
    const handleSettleClick = async (group) => {
        ...
        const amounts = group.members.map((member) =>
            web3.utils.toWei(member.splitAmount.toString(), "ether")
        );
        ...
        let total = formatEther(String(totalAmount))
        try {
            const provider = new ethers.BrowserProvider(window.ethereum);
            const signer = await provider.getSigner();
            const contracts = new ethers.Contract(contractAddress, abi, signer);
            console.log(totalAmount);
            const tx = await contracts.payMultiple(recipients, amounts, {
                from: signer.address,
